...
def check_requirements():
    ...
    if not github_token:
        issues.append("[FAIL] GITHUB_PERSONAL_ACCESS_TOKEN not set")
    else:
        print("[PASS] GitHub token found")
    ...
    try:
        ...
        issues.append("[FAIL] MCP SDK not installed: pip install mcp")
    ...
    if ai_provider == "huggingface":
        try:
            import transformers
            import torch
            print("[PASS] Hugging Face dependencies installed")
            
            # Check if GPU is available
            if torch.cuda.is_available():
                print(f"   [INFO] GPU available: {torch.cuda.get_device_name(0)}")
            else:
                warnings.append("[WARN] No GPU detected - model will run on CPU (slower)")
        except ImportError:
            issues.append("[FAIL] Hugging Face not installed: pip install transformers torch")
    elif ai_provider == "openai":
        try:
            import openai
            if not os.getenv("OPENAI_API_KEY"):
                issues.append("[FAIL] OPENAI_API_KEY not set")
            else:
                print("[PASS] OpenAI configured")
        except ImportError:
            issues.append("[FAIL] OpenAI not installed: pip install openai")
    elif ai_provider == "anthropic":
        try:
            import anthropic
            if not os.getenv("ANTHROPIC_API_KEY"):
                issues.append("[FAIL] ANTHROPIC_API_KEY not set")
            else:
                print("[PASS] Anthropic configured")
        except ImportError:
            issues.append("[FAIL] Anthropic not installed: pip install anthropic")
    ...
    try:
        ...
        if result.returncode == 0:
            print(f"[PASS] Docker installed: {result.stdout.strip()}")
            ...
            if result.returncode == 0:
                ...
                warnings.append("[WARN] Docker daemon not running: docker ps failed")
        else:
            warnings.append("[WARN] Docker not working properly")
    except FileNotFoundError:
        warnings.append("[WARN] Docker not found (will try binary)")
    except Exception as e:
        warnings.append(f"[WARN] Docker check failed: {str(e)}")
    
    # Check database
    try:
        import sqlalchemy
        print("[PASS] SQLAlchemy installed")
    except ImportError:
        issues.append("[FAIL] SQLAlchemy not installed")
...
async def test_mcp_connection():
    ...
    try:
        ...
        if client.is_available():
            print("[PASS] MCP server connected!")
            
            # Test a simple query
            print("\nTesting search_repositories...")
            result = await client.search_repositories("test", per_page=1)
            if result.get('items'):
                print("[PASS] Search test successful!")
            else:
                print("[WARN] Search returned no results (might be rate limited)")
            
            await client.close()
            return True
        else:
            print("[FAIL] MCP server not connected")
            print("\nTroubleshooting:")
        ...
        print(f"[FAIL] MCP connection failed: {str(e)}")
        print("\nTroubleshooting:")
...
async def test_ai_analyzer():
    ...
    try:
        ...
        if hasattr(ai_analyzer, 'model') and ai_analyzer.model:
            ...
            print("[PASS] AI API client configured")
            return True
        else:
            print("[WARN] AI model not loaded - will use heuristic fallbacks")
            return True
    except Exception as e:
        print(f"[FAIL] AI Analyzer initialization failed: {str(e)}")
        import traceback
        traceback.print_exc()
        return False
...
async def test_search():
    ...
    try:
        ...
        if repos:
            print(f"[PASS] Found {len(repos)} repositories:")
...
async def main():
    ...
    if not check_requirements():
        print("\n[FAIL] Please fix the issues above before continuing")
